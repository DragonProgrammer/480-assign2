...
#include <unistd.h>
#define Out 0                                                                   
 // Define the ends of the pipes so i do not have to remember numbers
#define In 1
#define BufferS 15 
// define buffer size
...
void ParentF(int PipeOutFD, int PipeInFD) { // Takes as a parameter the output
                                            // pipe end, and input pipe end
  char Buffer[BufferS];      // create the buffer
  ...
  printf("Parent process ready to run\nParent:\t\tValue = %ld\n",
         M);                // print stuff
  while (M < 99999999999) { // do this loop while M<99999999999
    if (read(PipeInFD, Value, BufferS) ==
        -1) {                       // etempt to read for mthe read end of pipe
...
void ChildF(int PipeOutFD, int PipeInFD) { // the child does the same as ParentF
                                           // withough an initial writing to pipe
  char Buffer[BufferS];
...
void GranF(
    int PipeOutFD,
    int PipeInFD) { 
	// Granchild funtion does the same as the Child function
  char Buffer[BufferS];
  char Value[BufferS] = "1";
  long M = 1;
...
int main(int argc, char **argv) {
  ...
  if (pipe(childtogran) ==
      -1) { // attempt to create child to granchild pipe, if not quit
  ...
  if (child) {              // in parent
    ...
    ParentF(partochild[In], grantopar[Out]); 
    // call parent function pass in pipe end to from
                             // granchild and to child
    ...
    if (gran) {
      ...
      close(grantopar[In]);
      close(grantopar[Out]);
      ChildF(childtogran[In],
             partochild[Out]); // call child function pass in pipe end from
                               // parent and to grandchild
      ...
      GranF(grantopar[In], childtogran[Out]); // call granchild function pass in
                                              // pipe to parent and from child
